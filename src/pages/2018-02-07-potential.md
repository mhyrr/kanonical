...
- Want to be faster?  Technique helps, but focus on combining speed workouts with endurance training.
- Want to be a better writer?  Eschew books about writing, and just write on a daily basis.
- Want to learn a math subject?  Build up enough knowledge (potential energy) to start practicing.  Then do problem sets.  Lots of them.
- Want to code?  Start with `println "Hello World!"` and iterate.  You can learn algorithmic complexity notation later.

This idea describes where I want to be: working.

##### Why is Potential Energy Dangerous?
...
I'm not trying to bash Armchair Experts.  I actually think it's a perfectly reasonable thing, as long as we're aware of the distinction.  Over time, most of us become armchair experts in a bunch of different subjects.  We need to be careful with this kind of expertise though, and treat it humbly.  This is especially true on subjects that marry expertise to identity, like [religion and politics](http://www.paulgraham.com/identity.html).  
...
I [mentioned recently](/mulatsag/) that Johnny von Neumann has become one of my personal heroes.  The guy was just on top of everything.
...
One of Johnny's seminal works that goes often overlooked is [*Probabilistic Logics and the Synthesis of Reliable Organisms from Unreliable Components*](http://fab.cba.mit.edu/classes/862.16/notes/computation/vonNeumann-1956.pdf).  In it, he described a mathematical approach for managing failure and error so that the desired logical outcome can still be achieved.  One of the keys to this - at a high level - is to stop relying on exactitude and to instead focus on probability.  In this way, there's a strong relationship with [Monte Carlo methods](https://en.wikipedia.org/wiki/Monte_Carlo_method) as well, which von Neumann was also intimately involved in.

When von Neumann's team was building the ENIAC in the 40s they had to use these ideas too, although they weren't yet formalized.  One of the main vacuum tube components of the [ENIAC](https://en.wikipedia.org/wiki/ENIAC) computer was the 6J6, a cheap, easy to purchase, and mostly reliable tube.  But errors and failures still happened frequently, to the point that [one guy](https://en.wikipedia.org/wiki/Julian_Bigelow) remarked that "the entire computer can be viewed as a big tube test rack."  
