...
All the ideas are there, but that's not it. If these neural nets really are building a world model - and I accept that they may be - there's going to consistently be an upper bound to the limit of their utility based on it. Humans need to be able to understand the model so that - like the shapes in a raytracing program - we can manipulate precise details of the model.